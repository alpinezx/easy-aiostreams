...
For moving AIOStreams to a new server.
...
- **Logged in as root:** the file lives in `/root`, and it prints something like
  ...
  ```
- **Logged in as yourself, using `sudo`:** the script also drops a copy in *your* home directory (owned by you), and prints something like
  ```bash
