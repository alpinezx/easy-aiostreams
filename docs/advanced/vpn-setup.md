...
**Note on what this test does and doesn't prove:** stopping gluetun
entirely tests the strongest guarantee — no container, no network stack,
period. It doesn't specifically exercise gluetun's own internal iptables
kill switch, which is the mechanism that matters for a more realistic
failure (the WireGuard tunnel dropping — an ISP blip, the VPN server going
unreachable — while the gluetun container itself keeps running). That's a
fiddlier scenario to trigger cleanly on demand. For the practical question
most people actually care about — "can a dead tunnel ever leak my real
IP" — the test above already answers it.
