...
notice() { printf '\033[1;33m!! %s !!\033[0m\n' "$1"; }
...
apply_mode() {
    ...
    if [[ "$mode" == "vpn" ]]; then
        ...
        echo ""
        local gluetun_startup_log
        gluetun_startup_log=$(docker compose logs gluetun --tail=15 2>&1)
        if grep -qi "connection reset by peer" <<< "$gluetun_startup_log" && grep -q ":853" <<< "$gluetun_startup_log"; then
            notice "Saw a WARN above about a DoT connection reset on :853. Normal,
gluetun runs two DNS resolvers (Cloudflare and Quad9) specifically so one
hiccuping on a single query doesn't matter, the other picks up the slack.
Only worth worrying about if 'DNS: encrypted' doesn't show up just below
this."
        fi
        ...
        if [[ "$skip_aiostreams" != "true" ]]; then
            if [[ "$tunnel_confirmed" == "true" ]]; then
                ...
                echo -e "\033[1;31mStarting it now could mean its traffic goes out unprotected. Check with\033[0m"
                echo -e "\033[1;31m'docker compose logs gluetun', fix the tunnel, then run this script again\033[0m"
                echo -e "\033[1;31mand choose 'Turn VPN ON' to retry. AIOStreams stays stopped until then.\033[0m"
            fi
        fi
    else
        if [[ "$skip_aiostreams" == "true" ]]; then
