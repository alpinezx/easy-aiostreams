...
# dashboard, and the config API.
#
# Can also manage an existing installation (status, stop, start, restart,
# update, uninstall). The management menu loops until you choose Exit.
#
# VPN-aware: if the optional gluetun VPN layer from setup-vpn-gluetun.sh is
...
# stay accurate regardless of what else runs first — re-resolving
...
# Detects whether the gluetun VPN layer is present in the live compose file
# (grepping for the container_name setup-vpn-gluetun.sh writes).
vpn_mode_active() {
    grep -q 'container_name: gluetun' "$COMPOSE_FILE" 2>/dev/null
}

# Warns before (re)starting AIOStreams when the VPN layer is installed but
# currently off. Silent if the VPN layer was never installed, or if it's
# currently on. Used by Start, Restart, Update, Reconfigure, Restore.
# Returns 0 to proceed, 1 if the user backed out.
...
# Returns 0 if confirmed connected, 1 if not confirmed after $1 attempts
# (default 5, ~10s at 2s apart).
...
# connected, and only then does aiostreams start. Used by Start, Restart,
# Update, Restore.
#
# Remaining args ("$@") are extra 'docker compose up' flags applied to both
# stages, e.g. --force-recreate --remove-orphans for Restart/Update.
#
# Returns 0 if aiostreams was started, 1 if withheld (tunnel not confirmed
# within 30s of gluetun starting, or gluetun itself never came up).
...
# ---------- shared plumbing (network + Caddy drop-ins) ----------
...
do_backup() {
    ...
    # Snapshot whether the watchdog's systemd timer is actually running RIGHT
    # NOW, at backup time — not just whether it was ever configured. Restore
    # reads this marker to decide whether to reinstall the timer on the new
    # server, so it only does so if watchdog was both present AND active
    # immediately before this backup, not merely set up at some point in the
    # past. The marker lives inside watchdog-state/, so it travels in the
    # tarball automatically like everything else here; always rewritten
    # (added or removed) on every backup so it never reflects a stale run.
    ...
    # Stage copies of the original WireGuard .conf file(s) for convenience,
    # so a migration doesn't mean re-uploading them by hand. These live
    # outside $INSTALL_DIR (per the documented setup steps: /root or the
    # invoking sudo user's home), so they'd otherwise never travel in the
    # tarball — the VPN layer itself doesn't need them after initial setup
    # (its actual working values already live in vpn-state/, which is
    # backed up regardless), this is purely so Restore can drop them back
    # in the same spot for quick server-switching later.
    #
    # Detection mirrors setup-vpn-gluetun.sh's own auto-detect exactly:
    # *.conf files directly in /root and $REAL_HOME (not recursive), kept
    # only if they actually look like a WireGuard config (has
    # [Interface]/[Peer]/PrivateKey/Endpoint) — so an unrelated .conf never
    # gets swept in. Filed under root/ or user/ by which home they came
    # from, not by filename, so Restore can put each back in the right
    # place even if the username differs on the new server.
    #
    # Re-staged fresh on every backup (old copies wiped first) so this never
    # reflects a file that's since been renamed, replaced, or removed.
...
# Runs after a backup is extracted but before Docker starts anything.
# Checks the restored Caddyfile's domain resolves to THIS server's public IP
# (a domain still pointing at the old server would make Caddy fail to get a
# cert). On mismatch, offers to re-check, wait/retry, fix the domain, or abort.
#
# DNS_RETRY_INTERVAL / DNS_RETRY_MAX (seconds) can be overridden by callers
# (e.g. tests) to avoid a real 10-minute wait.
check_restored_domain_dns() {
    ...
    while true; do
        ...
        # Several fallback IP-lookup services — any single one can be down.
        local public_ip svc
        public_ip=""
...
do_restore() {
    ...
    # Drop any staged original WireGuard .conf files back where they came
    # from (see do_backup). "root" files go to /root, "user" files go to
    # THIS server's $REAL_HOME — deliberately not the literal path recorded
    # on the old server, since the invoking username may differ here.
    # Convenience only; the VPN layer itself doesn't need this to work.
    local conf_stage_dir="$INSTALL_DIR/vpn-state/original-confs"
    ...
    if vpn_mode_active; then
        # Reinstall the boot-safety hook on THIS server before anything
        # starts — it lives outside $INSTALL_DIR so it never travels in the
        # backup. Without it, AIOStreams won't survive its next reboot
        # (it runs with restart: "no" in VPN mode; see setup-vpn-gluetun.sh).
        #
        # setup-vpn-gluetun.sh itself DOES travel in the backup (it's just a
        # file sitting in ~/aiostreams like everything else), so after
        # extraction the most likely place to find it is $INSTALL_DIR — NOT
        # next to this running script. Restore is typically launched from
        # $HOME (per the documented migration steps: the script + tarball
        # are scp'd straight to ~, not ~/aiostreams), so checking only
        # script_dir missed it every time a VPN layer was in play. Check
        # INSTALL_DIR first (where it actually lands from the backup), then
        # fall back to script_dir in case it was manually placed there
        # instead. Non-fatal if missing from both.
        local vpn_script=""
        ...
        if [[ -n "$vpn_script" ]]; then
            # < /dev/null: if $vpn_script is an OLDER copy that predates
            # this flag (it travels inside the backup, so it's only as
            # current as whatever was last deployed on the OLD server —
            # confirmed happening in practice, not just theoretical), an
            # unrecognized argument falls through into that script's
            # interactive menu instead of erroring. With stdin closed, its
            # first `read` hits EOF immediately and (set -e) the whole
            # thing exits non-zero right away — failing loudly instead of
            # silently hijacking this session waiting for menu input.
            bash "$vpn_script" install-boot-hook < /dev/null || true
            # Don't trust the exit code alone even so — a clean menu EXIT
            # (option 8) from an old, incompatible copy still returns 0
            # without ever actually installing anything. Check the real
            # thing that matters: does the systemd unit exist now?
    ...
    # Reinstall the watchdog's systemd timer if it was actually running at
    # backup time (see the was-active-at-backup marker written in
    # do_backup). Like the VPN boot hook above, the timer itself lives
    # outside $INSTALL_DIR and never travels in the backup, but
    # setup-watchdog.sh does (same reasoning as setup-vpn-gluetun.sh above),
    # so the same INSTALL_DIR-first, script_dir-fallback lookup applies.
    # Deliberately independent of the vpn_mode_active branch above — the
    # marker alone decides this, not current VPN state.
    ...
    if [[ -f "$INSTALL_DIR/watchdog-state/was-active-at-backup" ]]; then
        ...
        if [[ -n "$watchdog_script" ]]; then
            # Same reasoning as the VPN boot-hook call above: closed stdin
            # makes an incompatible old copy fail fast instead of falling
            # into its interactive menu, and the real check afterward is
            # whether the timer is actually active — not just whether the
            # subprocess happened to exit 0 (a clean menu EXIT from an old
            # copy does too, without installing anything).
        ...
        # Consumed — remove so it doesn't linger as a stale internal marker.
        rm -f "$INSTALL_DIR/watchdog-state/was-active-at-backup"
    fi
...
# on every single invocation — fresh install, restore, backup, menu, all of
# it. Without this, a very common pattern silently breaks backups forever:
# logging in as a non-root sudo user (the DEFAULT on DigitalOcean, Oracle,
# most cloud providers — e.g. the "ubuntu" user) and following the docs'
# own "cd ~/aiostreams && curl ..." convention downloads the script into
# THAT user's home — but $INSTALL_DIR under sudo is always root's home
# ($HOME is reset by sudo), a completely different directory. The script
# still runs fine either way, but do_backup only tars $INSTALL_DIR, so the
# running copy — and any later updates to it — would never actually reach
# the one place that gets backed up, unless someone manually copies it
# there every time. Confirmed happening in practice, repeatedly, not
# theoretical. This makes it self-healing instead: whatever copy you
# happen to be running becomes the canonical one, automatically, every run.
# Best-effort — never let a permissions hiccup here block the actual
# command the user ran. No-ops before the very first fresh install
# ($INSTALL_DIR doesn't exist yet) and once already in sync (cmp -s).
...
# ---------- direct subcommands ----------
# 'restore' must work before any install exists, so it's a subcommand
...
if [[ -f "$COMPOSE_FILE" ]]; then
    ...
    echo "  7) Uninstall (clean removal)"
    echo "  8) Backup (one tarball with everything needed for migration or safekeeping)"
    echo "  9) Restore from backup (replaces this install — see docs/basic/backup-restore.md)"
    echo " 10) Exit"
    echo ""
    ...
    case "$MENU_CHOICE" in
        ...
        4)
            echo ""
            cd "$INSTALL_DIR"
            # In VPN mode aiostreams shares gluetun's network namespace, so
            # it goes through the same gate as Start/Update/Restore rather
            # than recreating everything together and hoping the tunnel
            # catches up.
            if vpn_mode_active; then
            ...
            if [[ "$SWITCH_CHANNEL" =~ ^[Yy]$ ]]; then
                ...
                if [[ "$NEW_TAG" != "$CURRENT_TAG" ]]; then
                    ...
                    if [[ "$NEW_TAG" == "nightly" ]]; then
                        ...
                        warn "Consider backing up first (menu option 8) if you haven't already."
                    fi
        ...
        6)
            ...
            if vpn_mode_active; then
                warn "VPN mode is currently active (gluetun detected)."
                warn "Reconfiguring rewrites docker-compose.yml from a plain, non-VPN"
                warn "template — gluetun would be silently dropped and traffic would stop"
                warn "being tunnelled, even though the site itself would keep working."
                warn "The current config is backed up first either way, but VPN mode is"
                warn "NOT automatically restored — you'd need to re-run"
                warn "setup-vpn-gluetun.sh afterward to turn it back on."
                read -rp "Continue with reconfigure anyway? [y/N]: " CONFIRM_RECONFIGURE_VPN
        ...
        7)
            ...
            # Best-effort: only succeeds once nothing is attached to it.
...
if [[ -z "$RESOLVED_IP" ]]; then
    ...
    # Resolving isn't enough — it could be pointing at some other server.
    # Compare against this box's own public IP (best-effort, multiple
    # fallback lookup services).
    PUBLIC_IP=""
    for SVC in "https://ifconfig.me" "https://icanhazip.com" "https://ipinfo.io/ip"; do
        PUBLIC_IP=$(curl -fs4 --max-time 4 "$SVC" 2>/dev/null | tr -d ' \n') && [[ -n "$PUBLIC_IP" ]] && break
    done
...
# ---------- choose build channel (stable vs nightly) ----------

# Defaults to the currently-installed tag on reconfigure, or 'latest' for a
# fresh install. Same viren070/aiostreams:TAG line the compose template
# writes below, so this and that stay in sync.
...
# Extra confirmation ONLY when this actually changes the channel — typing 1/2
# again to reselect the same channel you're already on doesn't trigger it.
if [[ "$IMAGE_TAG" != "$DEFAULT_IMAGE_TAG" ]]; then
    ...
    if [[ "$IMAGE_TAG" == "nightly" ]]; then
        ...
        alert "If nightly has moved ahead with a config/database format change stable"
        alert "doesn't know about yet, stable may not read that data correctly."
        [[ -f "$COMPOSE_FILE" ]] && warn "Consider backing up first (menu option 8) if you haven't already."
    fi
...
echo "  - Next step in AIOStreams itself: go to Services, add your debrid provider's API key, then Addons > Marketplace to add a scraper."
echo ""
